<?php
// =============================================================================
//  CTI — I Know What You Download Module
//  Auth: key query param. Supports: ip
// =============================================================================
...
class IKnowWhatYouDownloadModule extends BaseApiModule
{
    private const API_ID   = 'iknowwhatyoudownload';
    private const API_NAME = 'IKnowWhatYouDownload';
    ...
    public function execute(string $queryType, string $queryValue, string $apiKey, string $baseUrl): OsintResult
    {
        ...
        $url = 'https://api.iknowwhatyoudownload.com/api/v1/peer/?' . http_build_query([
            'ip' => $queryValue,
            'key' => $apiKey,
        ]);
        ...
        $score = min(50, $count * 3);
        $severity = OsintResult::scoreToSeverity($score);

        $names = array_map(fn($c) => $c['name'] ?? '', array_slice($contents, 0, 5));
        $preview = implode(', ', array_filter($names));
        $summary = "IP {$queryValue}: {$count} torrent activities detected. Recent: {$preview}.";

        $tags = [self::API_ID, 'ip', 'torrent'];
        if ($count > 10) $tags[] = 'high_activity';
    ...
    {
        $url = 'https://api.iknowwhatyoudownload.com/api/v1/peer/?' . http_build_query(['ip' => '8.8.8.8', 'key' => $apiKey]);
