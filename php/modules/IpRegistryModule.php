<?php
// =============================================================================
//  CTI — ipregistry Module
//  Auth: key query param. Supports: ip
// =============================================================================

require_once __DIR__ . '/../HttpClient.php';
require_once __DIR__ . '/../OsintResult.php';
require_once __DIR__ . '/BaseApiModule.php';

class IpRegistryModule extends BaseApiModule
{
    private const API_ID   = 'ipregistry';
    private const API_NAME = 'ipregistry';
    private const SUPPORTED = ['ip'];

    public function execute(string $queryType, string $queryValue, string $apiKey, string $baseUrl): OsintResult
    {
        if (!in_array($queryType, self::SUPPORTED, true)) {
            return OsintResult::error(self::API_ID, self::API_NAME, "Unsupported: {$queryType}");
        }

        $url = 'https://api.ipregistry.co/' . urlencode($queryValue) . '?' . http_build_query(['key' => $apiKey]);
        $resp = HttpClient::get($url, [], 20);

        if ($resp['status'] === 429) return OsintResult::rateLimited(self::API_ID, self::API_NAME, $resp['elapsed_ms']);
        if ($resp['status'] === 401 || $resp['status'] === 403) return OsintResult::unauthorized(self::API_ID, self::API_NAME, $resp['elapsed_ms']);
        if ($resp['status'] === 404) return OsintResult::notFound(self::API_ID, self::API_NAME, $queryValue, $resp['elapsed_ms']);
        if ($resp['error'] || $resp['status'] === 0)
            return OsintResult::error(self::API_ID, self::API_NAME, $resp['error'] ?: 'Connection failed', $resp['elapsed_ms']);
        if ($resp['status'] !== 200)
            return OsintResult::error(self::API_ID, self::API_NAME, "HTTP {$resp['status']}", $resp['elapsed_ms']);

        $data = $resp['json'];
        if (!$data) return OsintResult::error(self::API_ID, self::API_NAME, 'Invalid JSON', $resp['elapsed_ms']);

        $country = $data['location']['country']['name'] ?? 'Unknown';
        $city = $data['location']['city'] ?? '';
        $org = $data['connection']['organization'] ?? 'Unknown';
        $asn = $data['connection']['asn'] ?? '';

        $security = $data['security'] ?? [];
        $isProxy = $security['is_proxy'] ?? false;
        $isVpn = $security['is_vpn'] ?? false;
        $isTor = $security['is_tor_exit'] ?? false;
        $threatLevel = $security['threat_level'] ?? 'low';

        $score = 5;
        if ($isProxy) $score += 20;
        if ($isVpn) $score += 15;
        if ($isTor) $score += 30;
        if ($threatLevel === 'high') $score += 30;
        elseif ($threatLevel === 'medium') $score += 15;
        $score = min(100, $score);

        $severity = OsintResult::scoreToSeverity($score);
        $tags = [self::API_ID, 'ip', 'geolocation'];
        if ($isProxy) $tags[] = 'proxy';
        if ($isVpn) $tags[] = 'vpn';
        if ($isTor) $tags[] = 'tor';

        $parts = ["IP {$queryValue}: Country: {$country}"];
        if ($city) $parts[] = "City: {$city}";
        $parts[] = "Org: {$org}";
        if ($asn) $parts[] = "ASN: {$asn}";
        $parts[] = "Threat: {$threatLevel}";

        return new OsintResult(
            api: self::API_ID, apiName: self::API_NAME,
            score: $score, severity: $severity, confidence: 85,
            responseMs: $resp['elapsed_ms'], summary: implode('. ', $parts) . '.',
            tags: $tags, rawData: $data, success: true
        );
    }

    public function healthCheck(string $apiKey, string $baseUrl): array
    {
        $url = 'https://api.ipregistry.co/8.8.8.8?' . http_build_query(['key' => $apiKey]);
        $resp = HttpClient::get($url, [], 10);
        if ($resp['error'] || $resp['status'] === 0) return ['status' => 'down', 'latency_ms' => $resp['elapsed_ms'], 'error' => $resp['error']];
        if ($resp['status'] === 401) return ['status' => 'down', 'latency_ms' => $resp['elapsed_ms'], 'error' => 'Invalid API key'];
        if ($resp['status'] === 200) return ['status' => 'healthy', 'latency_ms' => $resp['elapsed_ms'], 'error' => null];
        return ['status' => 'down', 'latency_ms' => $resp['elapsed_ms'], 'error' => "HTTP {$resp['status']}"];
    }
}
