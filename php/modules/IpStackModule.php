...
// =============================================================================
//  CTI — ipstack Module
//  API Docs: https://ipstack.com/documentation
//  Auth: access_key param. Supports: ip
//  Endpoint: http://api.ipstack.com/{ip}?access_key={key}
// =============================================================================
...
class IpStackModule extends BaseApiModule
{
    private const API_ID   = 'ipstack';
    private const API_NAME = 'ipstack';
    ...
    public function execute(string $queryType, string $queryValue, string $apiKey, string $baseUrl): OsintResult
    {
        ...
        $url = 'http://api.ipstack.com/' . urlencode($queryValue) . '?access_key=' . urlencode($apiKey) . '&output=json';
    ...
    {
        $resp = HttpClient::get('http://api.ipstack.com/8.8.8.8?access_key=' . urlencode($apiKey) . '&output=json', [], 10);
        if ($resp['error'] || $resp['status'] === 0) return ['status' => 'down', 'latency_ms' => $resp['elapsed_ms'], 'error' => $resp['error']];
        if ($resp['json']['error'] ?? false) return ['status' => 'down', 'latency_ms' => $resp['elapsed_ms'], 'error' => 'Invalid API key'];
